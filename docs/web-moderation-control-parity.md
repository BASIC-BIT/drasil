...
| Close open intake before submission  | Close Report button in intake thread                 | Reporter owns current open intake; confirmation; thread access   | `moderation_action_requests` queues bot execution through `ReportIntakeService.closeIntakeForThread`                                                                                | Mark intake closed; archive thread; no submitted report                                                                                  | `/report` reporter portal when open intake exists | Current queued handoff           |
