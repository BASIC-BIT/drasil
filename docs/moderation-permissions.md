...
| Close Report button in report intake threads          | Reporter or staff context          | Report-intake staff can be `ManageGuild`, `ModerateMembers`, or configured case responder roles | Re-checked per click by `ReportIntakeService.closeIntakeForThread`; role fallback is intentionally limited to report-intake thread moderation, not case opening.           |
