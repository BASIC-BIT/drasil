...
- Omit `case-role` to verify Drasil can reuse or create the default case role, or pass an existing role when testing role selection.
...
- Expected result:
  ...
  - detection-time setup warning DMs are best-effort, deduped, and do not expose suspicious message content
